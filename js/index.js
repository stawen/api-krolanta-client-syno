...
$(document).ready(function() {
    ...
    function makeBreadcrumb(url) {
        $("#breadcrumb").empty("");
        $('#breadcrumb').append(' <li><a href="#" class="folder" data-path=""><span class="glyphicon glyphicon-home"></span></a></li>');
        ...
        if (typeof url !== 'undefined') {
            ...
            $.each(ariane, function(key, val) {
                if (i !== nb) {
                    $('#breadcrumb').append('<li><a href="#" class="folder" data-path="">' + val + '</a></li>');
                } else {
    ...
    $("body").on("click", ".file", function(b) {
        var name = $(this).data('name');

        $.api('GET', 'addFile', { 'path': $(this).data('url') }, 'www').done(function(json) {
            //console.log(json);
            if (json) {
                $.growlValidate("Fichier ajouté : " + name);
            } else {
                $.growlErreur("Erreur dans l'ajout : " + name);
            }
    ...
    $("body").on("click", ".btn", function() {
        ...
        if ($(this).is("#mosaic")) {
            var img = $(this).closest("tr").find("td:nth-child(2)").children().data("url") + '.jpg';
            //console.log(img);
        ...
        if ($(this).is("#sample")) {
            var vid = $(this).closest("tr").find("td:nth-child(2)").children().data("url") + '.mp4';
            //console.log(vid);
